...
async def get_profiles_enriched(x_api_token: str | None = Header(None, alias="X-API-Token")):
    ...
    try:
        ...
        return {"action": "error", "message": "SSO enrichment timed out after 30 seconds"}
    except Exception as e:
...
async def get_console_url(
    ...
    try:
        ...
        return {"action": "error", "message": f"Console URL generation timed out for {profile_name}"}
    except Exception as e:
